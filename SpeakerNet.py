...
from loss.proxynca import ProxyNCA
from loss.proxyanchor import ProxyAnchor
from loss.masked_proxyanchor import MaskedProxyAnchor
from loss.masked_loss_balance_no_regular import MaskedLoss1
from loss.masked_proxynca import MaskedProxyNCA
from loss.proxyanchor_neg import ProxyAnchorNeg


from loss.mp_balance import MP_Balance #mp_balance
from loss.mmp_balance import MMP_Balance #mmp_balance
from loss.mmp_balance2 import MMP_Balance2 #mmp_balance2
from loss.mp import MP #mp


class SpeakerNet(nn.Module):

    def __init__(self, max_frames, lr = 0.1, margin = 1, scale = 1, hard_rank = 0, hard_prob = 0, model="alexnet50", nOut = 512, nSpeakers = 1000, optimizer = 'adam', encoder_type = 'SAP', normalize = True, trainfunc='contrastive', **kwargs):
        ...
        if trainfunc == 'angleproto':
            ...
            self.__test_normalize__     = True
        elif trainfunc == 'proxynca':
            self.__L__ = ProxyNCA(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        elif trainfunc == 'proxyanchor':
            self.__L__ = ProxyAnchor(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        elif trainfunc == 'masked_proxyanchor':
            self.__L__ = MaskedProxyAnchor(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
            
        elif trainfunc == 'masked_loss_balance_no_regular':
            self.__L__ = MaskedLoss1(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True   
            
        elif trainfunc == 'mp': #mp_balance
            self.__L__ = MP(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True  
            
        elif trainfunc == 'mp_balance': #mp_balance
            self.__L__ = MP_Balance(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True    
            
        elif trainfunc == 'mmp_balance': #mmp_balance
            self.__L__ = MMP_Balance(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True 
            
        elif trainfunc == 'mmp_balance2': #mmp_balance
            self.__L__ = MMP_Balance2(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True 
            
        elif trainfunc == 'masked_proxynca':
            self.__L__ = MaskedProxyAnchor(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
            
        elif trainfunc == 'proxyanchor_neg':
            self.__L__ = ProxyAnchorNeg(n_classes=nSpeakers).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        ...
        if optimizer == 'adam':
            self.__optimizer__ = torch.optim.Adam(self.parameters(), lr = lr, weight_decay=5e-5);
        elif optimizer == 'sgd':
        ...
        self.__scheduler__ = torch.optim.lr_scheduler.ReduceLROnPlateau(self.__optimizer__, mode='min', factor=0.7, patience=3, verbose=False)

    ## ===== ===== ===== ===== ===== ===== ===== =====
    ## Train network
    ## ===== ===== ===== ===== ===== ===== ===== =====

    def train_network(self, loader,veer):

        self.train();
        self.__scheduler__.step(veer)
        print(veer)
        stepsize = loader.batch_size
        print(self.__optimizer__)
    ...
    def evaluateFromListSave(self, listfilename, print_interval=5000, feat_dir='', test_path='', num_eval=10):
        ...
        for idx, line in enumerate(lines):
            ...
            #print(ref_feat.shape) (10, 512)
            #print(com_feat.shape) (10, 512)
