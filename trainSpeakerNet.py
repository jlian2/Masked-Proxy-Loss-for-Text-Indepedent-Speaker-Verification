...
from DatasetLoader_imbalance import DatasetLoader_imbalance
...
## Training details
parser.add_argument('--lr_interval', type=int, default=10, help='Test and save every [test_interval] epochs');
...
## Assertion
gsize_dict  = {'proto':2, 'triplet':2, 'contrastive':2, 'softmax':1, 'proxynca':1,'proxyanchor':1,'soft_proxyanchor':1, 'masked_proxyanchor':2,'masked_loss':2,'masked_loss_balance_no_regular':2, 'masked_loss_balance_regular':2,'masked_proxynca':2, 'proxyanchor_neg':1,'amsoftmax':1, 'aamsoftmax':1, 'ge2e':2, 'angleproto':2, 'mmp_balance':2, 'mp_balance':2,'mmp_balance2':2,'mp':1}
...
#trainLoader = DatasetLoader_imbalance(args.train_list, gSize=gsize_dict[args.trainfunc], **vars(args));
...
while(1):   
    ...
    ## Train network
    loss, traineer = s.train_network(loader=trainLoader,veer=veer);
