...
class ResNetSE(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(x.shape)  #(batch, 32240)  for mp(472,32240 once batch size is 200)
        #print(x.shape)  #(batch, 32240)  for mp_balance
        x = self.torchfb(x)+1e-6
        x = self.instancenorm(x.log()).unsqueeze(1).detach()
        
        #(batch * 1 * 40 * 202) #MelSpectrugram
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        if self.encoder_type == "SAP":
            ...
            #(batch * freq * channel * time)
            x = x.squeeze(dim=1).permute(0, 2, 1)  # batch * L * D (batch * time * channel)  # (batch * 51 * 128)
